...
ScatterPlot<-ggplot(data = airquality,aes(x = Temp,y = Wind,col = factor(Ozone)))+geom_point()
...
```
```{r show_figure1, fig.width = 9, fig.height = 5}
BarplotV1
BarplotV2
```
...
head(xycorr)
```
```{r show_figure2, fig.width = 6, fig.height = 5}
ScatterModel
```
